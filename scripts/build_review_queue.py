...
MANIFEST_PATH = PROJECT_ROOT / "papers" / "sources" / "current-program" / "manifest.csv"
...
def infer_priority(title: str, abstract: str, year: str) -> tuple[str, str]:
    ...
    if year == "2026":
        if matched:
            return "high", f"recent paper with relevant terms: {', '.join(matched[:5])}"
        return "medium", "recent paper but no strong viral/pathway keywords matched"
    ...
    if matched:
        return "medium", f"older paper with relevant terms: {', '.join(matched[:5])}"
    return "low", "no strong literature-priority keywords matched"
...
def main() -> int:
    rows = build_rows()
    write_rows(rows)
    print(f"Wrote {len(rows)} review queue rows to {REVIEW_QUEUE_PATH}")
    return 0
